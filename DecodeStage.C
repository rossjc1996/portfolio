...
//Sel+FwsA
    ...
    valA = getvalA(srcA, dreg, mreg, wreg, ex->getedstE(), ex->getevalE(), rf->readRegister(srcA, oops), ms->getvalM());
...
//FwdB
	...
	//];
    valB = getvalB(srcB, dreg, mreg, wreg, ex->getedstE(), ex->getevalE(), rf->readRegister(srcB, oops), ms->getvalM());
...
uint64_t DecodeStage::getvalB(uint64_t d_srcB,D * dreg, M * mreg, W * wreg, uint64_t e_dstE, uint64_t e_valE, uint64_t d_rvalB, uint64_t M_valM){
